...
review proceeds. **This is a routine information request, not a rejection.** Every
claim in the reply is checkable against the build, which is the point.

The reply itself is **`apple-1.3-reply.txt`** in this folder, kept as plain text
so it can go into the Connect thread without markdown fences or smart quotes
getting in the way. It lives in one place rather than two so the two cannot
drift apart.
