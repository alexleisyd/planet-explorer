...
**Body cache and texture lifetime (`index.html:1774`–`:1955`)** — a body does *not* bake once per session: a 2048×1024 map is resident twice over (the source canvas three keeps on `texture.image`, plus the GPU copy) at ~19.6 MB, so keeping all 24 would allocate ~800 MB and never return it. `built` is therefore an LRU of `BODY_CACHE` bodies (2 / 3 / 5 by device tier), and maps are **reference-counted** across it in `texRef` rather than owned by one body — a texture is shared, since Jupiter's build bakes its four moons and a moon's build bakes its host for the backdrop in its sky, so it is only freed when the last holder is evicted. Things that will bite:
