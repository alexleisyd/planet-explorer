...
- `buildBody` dedupes in flight through `building`. `setTarget` cannot guard this itself — `current` only moves once a bake finishes, so tapping one dot twice mid-bake would otherwise run two builds and orphan the loser's ref claims.
- The body on screen and the one just built are pinned in `trimCache`, so walking the selector strip faster than a bake completes can never evict what it is about to show. Peak during that churn is still bounded by *in-flight builds*, not by `BODY_CACHE`.
